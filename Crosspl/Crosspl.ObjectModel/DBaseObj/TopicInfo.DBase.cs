    ...
    public partial class TopicInfo : IDBEntity, Identifiable
    {
            ...
            int topWritersLimit)
        {
            ...
            if (topics.Count > 0)
            {
                ...
                {
                    FacebookUser createdBy = CrossplUtils.Search<FacebookUser>(facebookUsers, topic.CreatedBy);
                    topic.Creator = createdBy;

                    foreach (long writerId in topic.TopWriterIDs)
                    {
                        FacebookUser contributedBy = CrossplUtils.Search<FacebookUser>(facebookUsers, writerId);
