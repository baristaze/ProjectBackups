----------------------------------------------------------------------------
----------------------------------------------------------------------------
IF EXISTS (SELECT [Name] FROM sys.objects WHERE [Type] = 'p' AND [Name] = 'GetUpVotedEntries')
BEGIN
	EXEC('DROP PROCEDURE [dbo].[GetUpVotedEntries]');
END
GO

CREATE PROCEDURE [dbo].[GetUpVotedEntries]
 @TopicId bigint
...
-- always from popular to poor ones
AS
	-- ////////////////////////////////////////////////////////////////////////////////////////////////////--
	-- get all entries under this topic
	...
	(
		SELECT [E].[Id]
			...
			WHERE [E].[Id] = @IncludedEntryId
	UNION ALL -- no duplicate for sure. that is why all
		SELECT [E].[Id]
			  ,[E].[Status]
			  ,[E].[TopicId]
			  ,[E].[Content]
			...
			LEFT JOIN @reactionResults [R] ON [E].[Id] = [R].[EntryId]
			WHERE [E].[Id] <> @IncludedEntryId
			ORDER BY [V].[NetVoteSum] DESC OFFSET @Offset ROWS FETCH NEXT @NextFetchCount ROWS ONLY
		) AS [All];
GO
