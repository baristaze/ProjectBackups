        ...
        if (statusText == "success") {
            if (response.ErrorCode == "0") {
                ...
                if (response.Topics.length <= 0) {
                    dataArray[index++] = { label: "No result found", value: searchTerm, objectId: -200 };
                }

                // add 'add new' stuff
                if (offerAddNew) {
                    dataArray[index++] = { label: "Add New", value: searchTerm, objectId: -100 };
                }
...
function renderAutoCompleteItem(ul, item) {
    ...
    if (item.objectId == -100) {
        return $("<li class='ui-menu-item ui-add-new'></li>")
                      ...
                      .append("<a><span class='search-add-new-txt'>" + item.value + "</span><button class='rnd-crn search-add-new-btn'>+ add this topic</button></a>")
                      .appendTo(ul);
    }
        ...
        return $("<li class='ui-menu-item'></li>")
                          .data("item.autocomplete", item)
                          .append("<a><span class='search-no-result'>No result found</span></a>")
                          .appendTo(ul);
    }
...
function topicAddSucceededEx(jsonData) {
    return function topicAddSucceeded(response, statusText, xhr, $form) {
        ...
        if (statusText == "success") {
            ...
            topicAddFailedEx("Unknown error occured while creating the topic!")();
        }
    }     // core 
} // closure
...
function sendAddTopicRequest(topicTitle, shareOnFacebook, shareOnTwitter) {
    ...
    $.ajax({
        ...
        error: topicAddFailedEx("Unknown error occured while creating the topic!")
    });
...
function topicDeleteSucceeded(response, statusText, xhr, $form) {
    ...
    if (statusText == "success") {
        ...
        topicDeleteFailedEx("Unknown error occured while deleting the topic!")();
    }
}

function deleteTopic() {
    showBlockedConfirmDialog("Are you sure to delete this topic?", function (dlgRes) {
        if (dlgRes == 1) {
            ...
            $.ajax({
                ...
                error: topicDeleteFailedEx("Unknown error occured while deleting the topic!")
            });
        }
    });
}
