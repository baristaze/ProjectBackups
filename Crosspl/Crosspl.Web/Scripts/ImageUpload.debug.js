...
function _adjustWatermarkDesc($textArea, flag, blurTextArea) {
    if (flag == 1 || flag == 2 || flag == 3) {
        ...
        if (flag == 1) {
            descrArea.attr("value", "add your entry");
        }
        else if (flag == 2) {
            descrArea.attr("value", "say something about this photo\n\n");
        }
        else if (flag == 3) {
            descrArea.attr("value", "say something about this photo\n\ntell more about the above photo");
        }
...
function fileUploadSucceededEx($btn, $form, $errorArea, imageSource, imageUrlFromWeb) {
    return function fileUploadSucceeded(responseText, statusText, xhr, $form) {
        if (statusText == "success") {
            ...
            fileUploadFailedEx($btn, $form, $errorArea, "Unknown error occured", imageSource, imageUrlFromWeb)();
        }
    }                             // core
} // closure
...
function uploadPhoto($btn, $form, $errorArea, imageSource, imageUrlFromWeb) {
    ...
    var options = {
        ...
        error: fileUploadFailedEx($btn, $form, $errorArea, "Unknown error occured", imageSource, imageUrlFromWeb),
...
function startUploadingPhoto($btn) {
    ...
    if (imageSource == 0) {
        ...
        if (imageUrlFromWeb.length <= 0) {
            showUploadError(errorArea, "Please enter the web link of the image");
            return false;
        }
    }
    else if (imageSource == 1) {
        ...
        if (filePath) {
            if (!checkFileExt(filePath, __allowedExtensions)) {
                showUploadError(errorArea, "Unsupported file type");
                return false;
            }
        }
        else {
            showUploadError(errorArea, "Please choose a file");
            return false;
        }
    }
    else {
        showUploadError(errorArea, "Please choose one of the radio buttons");
        return false;
    }
