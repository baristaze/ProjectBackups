...
namespace Shebeke.ObjectModel
{
    public abstract class ConfigBase
    {
        ...
        protected string Get(string key, bool throwOnError, string defaultVal)
        {
            ...
            if (String.IsNullOrWhiteSpace(val))
            {
                ...
                if (throwOnError)
                {
                    ...
                    throw new ShebekeException(msg);
                }
                else
                {
        ...
        protected int GetAsInt(string key, bool throwOnError, int defaultVal, int min, int max)
        {
            ...
            if (val < min)
            {
                throw new ShebekeException(key + " may not be smaller than " + min.ToString());
            }

            if (val > max)
            {
                throw new ShebekeException(key + " may not be bigger than " + max.ToString());
            }
