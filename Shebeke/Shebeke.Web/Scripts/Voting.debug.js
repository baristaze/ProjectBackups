    ...
    return function voteSucceeded(response, statusText, xhr, $form) {
        ...
        if (statusText == "success") {
            if (response.ErrorCode == "0") {
                ...
                var doneInfo = $(resultArea).find(".act-done");
                if (response.VotingSummary.MyVote != 0) {
                    doneInfo.text("Başarıyla kaydedildi!");
                }
                else {
                    doneInfo.text("Geri çekildi!");
                }
                doneInfo.show("fast", "linear", function () {
            ...
            voteFailedEx(btn, vote, resultArea, "Oyunuz kaydedilirken beklenmeyen bir hata oluştu!")();
        }
    }       // core 
} // closure function
...
function sendVote(btn, vote) {
    ...
    $.ajax({
        ...
        error: voteFailedEx(btn, vote, resultArea, "Oyunuz kaydedilirken beklenmeyen bir hata oluştu!")
    });
...
function showBlockedComparisonDialog() {
    ...
    var title = 'Slm ' + user + ', oy ver, en iyi paylaşımları seçmemizi sağla!';
    ...
    if (username && photoUrl) {
        ...
        title = username + ': "Slm ' + user + ', hangi içerik daha iyi?' + '"';
...
function entryPage_Init_Voting() {
    $(".vote").live('click', function () {
        ...
        if ($(this).hasClass("vote-compare")) {
            ...
            var title = 'Facebook - Arkadaş Daveti';
            if ($(window).width() > 540) {
                title = 'Facebook - Seçimini paylaşmak için TÜM arkadaşlarını seç!';
            }
