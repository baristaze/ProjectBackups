...
function shareEntryOnFacebook($childItem) {
    ...
    if (entryContentText.length > 5000) {
        entryContentText = entryContentText.substring(0, 5000)
    }
    ...
    if (entryImage.length > 0) {
        ...
        entryFirstImageLink = getRootUrl() + "/Images/logo.png?version=0.81";
    }
        ...
        if (statusText == "success") {
            if (response.ErrorCode == "0") {
                if (sentCountSoFar == selectedFriendsAll.length) {
                    // last trip
                    $(".invite-friends-to-topic").text("Diğer arkadaşlarınızı da davet edin");
                }
            }
        }
    }
...
function showInvitationSendWindow(topicId, entryId, selectedFriendsAll, sentCountSoFar) {
    if (selectedFriendsAll.length > sentCountSoFar) {
        ...
        FB.ui({ method: 'apprequests',
            ...
            message: 'Bu konuya katkıda bulunabilirsin',
...
function fbInvitationConfirmed($customDlg, topicId, entryId) {
    ...
    if (selectedFriends.length == 0) {
        alert("lütfen en az bir arkadaş seçiniz");
        return false;
    }
    ...
    $dlg.dialog(
                {
                    buttons:
                    {
                        "SendRequests": { 'text': 'Davet Et', 'class': 'facebookInviteButton', 'click': function () { fbInvitationConfirmed($(this), topicId, entryId); } },
                        "Cancel": { 'text': 'İptal', 'class': 'facebookInviteButton facebookInviteCancelButtonClass', 'click': function () { fbInvitationCancelled($(this), topicId, entryId); } }
                    }
                }
            );

    if ($(window).width() < 540) {
        // we don't have enough space for this on mobile
        var filterArea = $("#facebookFriendsFilterPane");
        filterArea.hide();
    }
...
function retrieveFacebookFriends($btn, topicId, entryId, includePics, dialogTitle) {
    if ($btn) {
        $btn.find(".invite-friends-to-topic").text("Lütfen bekleyin...");
    }
    ...
    if (friends.length > 0) {
        ...
        if ($btn) {
            $btn.find(".invite-friends-to-topic").text("Arkadaşlarını katkıda bulunmaya davet et!");
        }
    }
    else {
...
function inviteFriendsToTopic($btn) {
    var topicId = $("#topicId").text();
    retrieveFacebookFriends($btn, topicId, 0, true, 'Facebook - Arkadaş Daveti');
}
