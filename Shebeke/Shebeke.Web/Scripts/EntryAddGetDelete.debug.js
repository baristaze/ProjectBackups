...
function showHideAddButtonSpinner($btn, isShow) {
    if (isShow) {
        ...
        $btn.html("<img class='act-busy' alt='...' src='" + resourceLink + "' /><span>&nbsp;&nbsp;ekleniyor...</span>");
    }
    else {
        $btn.html("");
        $btn.text("+ ekle");
...
function entryAddSucceededEx(jsonData, $textArea, $btn, $photoUploadSection, insertAtTop) {
    return function entryAddSucceeded(response, statusText, xhr, $form) {
        ...
        if (statusText == "success") {
            ...
            entryAddFailedEx($textArea, $btn, $photoUploadSection, "Paylaşımınız kaydedilirken beklenmeyen bir hata oluştu!")();
        }
    }       // core 
} // closure function
...
function addEntry(text, $textArea, $btn) {
    ...
    $.ajax({
        ...
        error: entryAddFailedEx($textArea, $btn, photoUploadSection, "Paylaşımınız kaydedilirken beklenmeyen bir hata oluştu!")
    });
...
function loadingEntriesSucceeded(response, statusText, xhr, $form) {
    ...
    if (statusText == "success") {
        ...
        loadingEntriesFailedEx("Sayfa içeriği yüklenemedi. F5 tuşuna basarak sayfayı yenilemeyi deneyin!")();
    }
} // core 
...
function loadInitialEntries() {
    ...
    $.ajax({
        ...
        error: loadingEntriesFailedEx("Sayfa içeriği yüklenemedi. F5 tuşuna basarak sayfayı yenilemeyi deneyin!")
    });
}
...
function entryDeleteSucceededEx($btn, $mainBlock) {
    return function entryDeleteSucceeded(response, statusText, xhr, $form) {
        ...
        if (statusText == "success") {
            ...
            entryDeleteFailedEx($btn, "Kayıt silinirken beklenmeyen bir hata oluştu!")();
        }
    }               // core 
} // closure function
...
function deleteEntry($btn) {
    ...
    showBlockedConfirmDialog("Bu kaydı silmek istediğinizden emin misiniz?", function (dlgRes) {
        if (dlgRes == 1) {
            ...
            $.ajax({
                ...
                error: entryDeleteFailedEx($btn, "Kayıt silinirken beklenmeyen bir hata oluştu!")
            });
        }
    });
}
