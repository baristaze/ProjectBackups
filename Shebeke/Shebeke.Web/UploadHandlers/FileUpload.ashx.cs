...
namespace Shebeke.Web
{
    ...
    /// </summary>
    public class FileUpload : ShebekeHttpHandlerJson<UploadedFile>
    {
        ...
        private bool IsFromLocalFile(HttpContext context)
        {
            ...
            {
                throw new ShebekeArgumentException("Invalid flag", "IsLocalFile");
            }
            else
            {
        ...
        private UploadedFile ProcessFileUpload(HttpContext context, bool isLocalFile, MemoryStream memoryStream, UserAuthInfo user)
        {
            ...
            using (SqlConnection conn = new SqlConnection(config.DBaseConnectionString))
            {
                ...
                if (user.UserId > 0)
                {
                    this.CheckDenialOfService(conn, null, user.UserId, UserActivityType.UploadImage);
                }
            }
            ...
            {
                throw new ShebekeException("There is no file info");
            }
            ...
            finally
            {
                ...
                if (user.UserId > 0)
                {
                    using (SqlConnection conn = new SqlConnection(config.DBaseConnectionString))
                    {
                        ...
                        if (user.UserId > 0)
                        {
                            this.LogUserActivity(conn, null, user.UserId, UserActivityType.UploadImage);
                        }
                    }
                }
        ...
        private void SetImageDimensions(MemoryStream memoryStream, ImageFile imageFile)
        {
            ...
            {
                throw new ShebekeException("Image couldn't be processed as bitmap", ex);
            }
        }
        ...
        private void GetFileStreamFromPost(HttpContext context, MemoryStream memoryStream, out ImageFile imageFile)
        {
            ...
            {
                throw new ShebekeArgumentException("File is empty", "LocalFile");
            }

            string ext = Path.GetExtension(httpPost.FileName);
            if (String.IsNullOrWhiteSpace(ext))
            {
                throw new ShebekeArgumentException("File type is undefined", "LocalFile");
            }

            ext = ext.Trim(' ', '.', '*').ToLower();
            if (!ImageFile.AllowedExtensions.Contains(ext, StringComparer.OrdinalIgnoreCase))
            {
                throw new ShebekeArgumentException("File type is not allowed", "LocalFile");
            }
        ...
        private void GetFileStreamFromWeb(HttpContext context, string fileFromWeb, MemoryStream memoryStream, out ImageFile imageFile)
        {
            ...
            {
                throw new ShebekeException("The provided URL doesn't point to an image", ex);
            }

            if (response.ContentLength <= 0)
            {
                throw new ShebekeArgumentException("The content pointed by the provided URL is empty");
            }

            if (response.ContentType == null || !response.ContentType.StartsWith("image/", StringComparison.OrdinalIgnoreCase))
            {
                throw new ShebekeArgumentException("The content pointed by the provided URL is not an image");
            }
