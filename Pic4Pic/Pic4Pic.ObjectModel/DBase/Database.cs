...
namespace Pic4Pic.ObjectModel
{
    public class Database
    {
        ...
        public delegate void DatabaseOperation();
        ...
        public static int InsertOrUpdate(IEnumerable<IEditable> entities, string connectionString, int timeOuts)
        {
            ...
            if (!allSucceeded)
            {
                throw new Pic4PicException("Couldn't commit to the database");
            } 
        ...
        public static void PerformAndCommit_or_RollbackAndThrow(SqlTransaction trans, DatabaseOperation operationDelegate)
        {
            Exception ex = null;
            try
            {
        ...
        public static Guid GetGuidOrDefault(SqlDataReader reader, ref int index)
        {
            Guid val = default(Guid);
            if (!reader.IsDBNull(index))
            {
                val = reader.GetGuid(index);
            }
            index++;
            return val;
        }
