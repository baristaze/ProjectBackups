...
namespace Pic4Pic.ObjectModel
{
    ...
    {
        protected static RSAParameters GetRsaParameters_2048_Bit_PKCS1_v1_5(byte[] publicKey)
        {
        ...
        public static bool VerifySignature_2048_Bit_PKCS1_v1_5(byte[] data, byte[] signature, byte[] publicKey)
        {
            ...
            using (SHA1 sha1 = SHA1.Create()) 
            {
                ...
                using (RSACryptoServiceProvider rsa = new RSACryptoServiceProvider(2048))
                {
                    ...
                    {
                        throw new Pic4PicException("Unexpected exception when checking the purhase signature. Importing RSA parameters failed: " + ex.Message);
                    }
                    ...
                    try
                    {
                        verified = rsaDeformatter.VerifySignature(hash, signature);
                    }
                    catch (System.Security.Cryptography.CryptographicUnexpectedOperationException ex)
                    {
                        throw new Pic4PicException("Unexpected exception when checking the purhase signature: " + ex.Message);
                    }
