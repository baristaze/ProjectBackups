...
public class PurchaseUtils {
	...
	protected static PurchaseRecordList removePurchaseFromFile(Activity activity, PurchaseRecord purchaseRecord, int keyId, String type) throws GingerException {
		...
		String purchasesAsJsonText = purchasesFile.getString(key, "");
		if(purchasesAsJsonText == null || purchasesAsJsonText.trim().length() == 0){
			MyLog.w("PurchaseUtils", "There is not any saved " + type + " purchase to remove.");
			return new PurchaseRecordList();
		}
		...
		for(PurchaseRecord existing : purchases){
			if(existing.getPurchaseReferenceToken().equals(purchaseRecord.getPurchaseReferenceToken())){
				MyLog.i("PurchaseUtils", "The " + type + " purchase has been found and will be removed: " + purchaseRecord.getPurchaseReferenceToken());
		...
		if(!found){
			MyLog.w("PurchaseUtils", "The " + type + " purchase with token = " + purchaseRecord.getPurchaseReferenceToken() + " couldn't be found to remove");
			return new PurchaseRecordList();
		}
	...
	public static int consumePurchaseOnAppStoreAndClearLocal(final Activity activity, final InAppPurchasingService purchasingService, final PurchaseRecord purchase){
		...
		try {
			...
			MyLog.e("PurchaseUtils", "Consuming purchase is failed for the purchase token: " + purchase.getPurchaseReferenceToken());
		}
		
		// update the file
		if(consumed) {
			try {
				MyLog.v("PurchaseUtils", "Removing the unconsumed purchase record from file: " + purchase.getPurchaseReferenceToken());
				PurchaseUtils.removeUnconsumedPurchaseFromFile(activity, purchase);
				return 2;
			} 
			catch (GingerException e) {
				MyLog.e("PurchaseUtils", "Removing the unconsumed purchase record from file failed: " + e.toString());
				return 1;
			}
		}
		else{
